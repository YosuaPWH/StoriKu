...
class ImageStoryWidget : AppWidgetProvider() {
    ...
    override fun onReceive(context: Context?, intent: Intent) {
        ...
        if (intent.action != null) {
            if (intent.action == CLICK_ACTION) {
                val viewIndex = intent.getIntExtra(STORY_IMAGE, 0)
                Toast.makeText(context, "TOUCHED STORY $viewIndex", Toast.LENGTH_SHORT).show()
    ...
    companion object {
        ...
        const val STORY_UPDATE = "story_update"
        ...
        private fun updateAppWidget(context: Context, appWidgetManager: AppWidgetManager, appWidgetId: Int) {
            ...
            val views = RemoteViews(context.packageName, R.layout.layout_story_widget)
